...
/**
 * 【期末模拟发现的错误】应该解耦《语义错误处理》和《代码生成》
 * 语义错误处理器
 * 仅用于检查语义错误，若有语义错误则不输出中间代码、目标代码
 * 因此保证了输出中间代码时，不会出现语义错误
 */
public class SemanticErrorHandler {
    private SemanticErrorHandler() {}

    private static final SemanticErrorHandler handler = new SemanticErrorHandler();

    public static SemanticErrorHandler getInstance() {
        return handler;
    }

    private SymbolTable curSymbolTab;
    ...
    private void visitConstDef(ConstDef constDef, Btype btype) {
        ...
        if (constDef.getConstExp() == null) {
            ...
            if (btype.getBtypeToken().getTokenType() == Token.TokenType.INTTK) {
                curSymbolTab.addSymbol(new ConstIntSymbol(token.getValue(), 0));
            } else {
                curSymbolTab.addSymbol(new ConstCharSymbol(token.getValue(), '\0'));
            }
        } else {
            // 常量数组
            visitConstExp(constDef.getConstExp());
            if (btype.getBtypeToken().getTokenType() == Token.TokenType.INTTK) {
                curSymbolTab.addSymbol(new ConstIntArraySymbol(token.getValue(), new int[0]));
            } else {
                curSymbolTab.addSymbol(new ConstCharArraySymbol(token.getValue(), new char[0]));
            }
        }
    }
